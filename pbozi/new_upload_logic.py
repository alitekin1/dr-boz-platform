...
async def _perform_direct_extraction(update, context, user, chat_id, doc, file_path, filename, text, ext, processing_msg):
    async with async_session() as db:
        if not chat_id:
        ...
        msg_content = f"[عکس: ID={uploaded_file_id}]" if ext in ("jpg", "jpeg", "png", "webp") else f"[فایل: {filename}]"
    ...
    if not update.message.media_group_id:
        if update.message.caption: await handle_message(update, context, forced_text=update.message.caption)
        else: await processing_msg.edit_text(f"✅ فایل {filename} دریافت شد. چه سوالی داری؟")

async def _perform_rag_indexing(update, context, user, project_id, chat_id, doc, file_path, filename, file_size, ext, explicit_upload_requested, processing_msg):
    from app.rag import _read_file
    try:
        file_text = await asyncio.to_thread(_read_file, file_path)
...
async def _handle_audio_doc_inline(update, context, user, project_id, chat_id, doc, file_path, filename, ext, explicit_upload, processing_msg):
    ...
    if transcript and explicit_upload:
        ...
        async with async_session() as db:
            ...
            asyncio.create_task(_background_index_with_notification(bot=context.bot, chat_id=update.effective_chat.id, project_id=project_id, document_id=doc_record.id, file_path=transcript_path, uid=user.telegram_id, update_id=update.update_id, file_unique_id=doc.file_unique_id, estimated_cost=0, estimated_tokens=_estimate_text_tokens(transcript), filename=filename, api_key=emb.api_key if emb else None, model=emb.model if emb else None, provider=emb.provider if emb else "google", base_url=emb.base_url if emb else None, processing_msg_id=processing_msg.message_id))
