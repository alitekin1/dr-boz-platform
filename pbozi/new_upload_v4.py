...
async def _handle_document_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            # For non-large plain text files, read directly
            if ext in ("txt", "md", "json", "py", "js", "sql", "csv"):
                file_text = await asyncio.to_thread(_read_file, file_path)
                if len(file_text) > 15000: is_large = True
            else:
                is_large = True # PDF, Office, etc. -> always treat as potential RAG if needed, but let's check size
        ...
        # Use RAG if: explicit for project, OR file is large PDF/Office
    ...
    if mg_id:
        ...
        if caption:
            ...
            await processing_msg.edit_text(
                f"✅ فایل {filename} دریافت شد ({q_len} فایل در لیست).\n\n"
...
async def _perform_rag_indexing(update: Update, context: ContextTypes.DEFAULT_TYPE, user, project_id, chat_id, doc, file_path, filename, file_size, ext, explicit_upload_requested, processing_msg):
    ...
    if mg_id:
        ...
        if caption:
            ...
            q_len = len(context.user_data["pending_files_queue"])
            await processing_msg.edit_text(
                f"✅ {filename} (فایل سنگین) آماده شد ({q_len} فایل در لیست).\n\n"
                "می‌تونی فایل‌های دیگه‌ای بفرستی یا با زدن دکمه زیر شروع به گفتگو کنی 👇",
                reply_markup=upload_queue_kb()
            )
