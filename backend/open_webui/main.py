...
from open_webui.config import (
    ...
    reset_config,
    async_reset_config,
    # Bot Auth PersistentConfig
    TELEGRAM_BOT_TOKEN_PERSISTENT,
    TELEGRAM_BOT_USERNAME_PERSISTENT,
    BALE_BOT_TOKEN_PERSISTENT,
    BALE_BOT_USERNAME_PERSISTENT,
    AUTH_CODE_TTL_PERSISTENT,
    # Billing PersistentConfig
    MODEL_PRICING_CONFIG,
    DEFAULT_COOLDOWN_LIMIT_TOMAN,
    DEFAULT_WEEKLY_LIMIT_TOMAN,
...
v{VERSION} - Dr. Boz AI Chat Platform
{f'Commit: {WEBUI_BUILD_HASH}' if WEBUI_BUILD_HASH != 'dev-build' else ''}
https://github.com/drboz/dr-boz
""")


async def _ensure_user_phone_column():
    """Idempotent: add User.phone column if the model declares it but the
    table was created in a previous version that didn't have it."""
    try:
        ...
        def _has_phone(conn):
            ...
            return any(c['name'] == 'phone' for c in cols)

        with engine.begin() as conn:
            if not _has_phone(conn):
                log.info('Migration: adding user.phone column')
                # SQLite and PostgreSQL both accept this minimal type
                conn.execute(text('ALTER TABLE "user" ADD COLUMN phone VARCHAR(32)'))
    except Exception as e:
        log.warning(f'_ensure_user_phone_column skipped: {e}')
...
async def lifespan(app: FastAPI):
    ...
    # Lightweight schema migrations: add columns that the model declares but
    # Alembic does not auto-apply. Safe to run on every boot; no-ops if the
    # column already exists.
    await _ensure_user_phone_column()
...
app.state.config.TELEGRAM_BOT_TOKEN = TELEGRAM_BOT_TOKEN_PERSISTENT
app.state.config.TELEGRAM_BOT_USERNAME = TELEGRAM_BOT_USERNAME_PERSISTENT
app.state.config.BALE_BOT_TOKEN = BALE_BOT_TOKEN_PERSISTENT
app.state.config.BALE_BOT_USERNAME = BALE_BOT_USERNAME_PERSISTENT
app.state.config.AUTH_CODE_TTL = AUTH_CODE_TTL_PERSISTENT

# Billing configs (Dr. Boz SAAS)
app.state.config.MODEL_PRICING_CONFIG = MODEL_PRICING_CONFIG
app.state.config.DEFAULT_COOLDOWN_LIMIT_TOMAN = DEFAULT_COOLDOWN_LIMIT_TOMAN
app.state.config.DEFAULT_WEEKLY_LIMIT_TOMAN = DEFAULT_WEEKLY_LIMIT_TOMAN
app.state.config.DEFAULT_COOLDOWN_HOURS = DEFAULT_COOLDOWN_HOURS
...
app.include_router(bot_auth.router, prefix='/api/v1/bot-auth', tags=['bot-auth'])
app.include_router(otp_auth.router, prefix='/api/v1/otp-auth', tags=['otp-auth'])
...
async def chat_completion(
    ...
    # ── ERA Limit Check (Dr. Boz SAAS) ──────────────────────
    if user.role != 'admin':
        try:
            from open_webui.utils.era_db import is_era_db_available
            if await is_era_db_available():
                from open_webui.routers.account import _get_account_usage_internal
                era = await _get_account_usage_internal(user)
                limits = era.get('limits', {})
                five_hour = limits.get('five_hour', {})
                seven_day = limits.get('seven_day', {})
                sub_info = era.get('subscription', {})

                if sub_info.get('status') not in (None, 'active'):
                    raise HTTPException(
                        status_code=402,
                        detail='اشتراک شما منقضی شده است. لطفاً اشتراک خود را تمدید کنید.',
                    )

                if five_hour.get('reached', False):
                    extra = ''
                    if five_hour.get('reset_at'):
                        try:
                            from datetime import datetime, timezone
                            reset_dt = datetime.fromisoformat(five_hour['reset_at'].replace('Z', '+00:00'))
                            diff = (reset_dt - datetime.now(timezone.utc)).total_seconds()
                            if diff > 0:
                                hrs = int(diff // 3600)
                                mins = int((diff % 3600) // 60)
                                extra = f' {hrs} ساعت و {mins} دقیقه دیگر'
                        except Exception:
                            pass
                    raise HTTPException(
                        status_code=402,
                        detail=f'محدودیت ۵ ساعته مصرف شما به پایان رسیده است.{extra}',
                    )

                if seven_day.get('reached', False):
                    extra = ''
                    if seven_day.get('reset_at'):
                        try:
                            from datetime import datetime, timezone
                            reset_dt = datetime.fromisoformat(seven_day['reset_at'].replace('Z', '+00:00'))
                            diff = (reset_dt - datetime.now(timezone.utc)).total_seconds()
                            if diff > 0:
                                days = int(diff // 86400)
                                hrs = int((diff % 86400) // 3600)
                                extra = f' {days} روز و {hrs} ساعت دیگر'
                        except Exception:
                            pass
                    raise HTTPException(
                        status_code=402,
                        detail=f'محدودیت هفتگی مصرف شما به پایان رسیده است.{extra}',
                    )
            else:
                # Fallback to Simple Credit System when ERA DB unavailable
                from open_webui.models.subscriptions import Subscriptions
                from open_webui.models.user_credits import Credits

                credits = await Credits.check_and_reset_monthly(user.id)
                sub = await Subscriptions.get_subscription_by_user_id(user.id)

                if not sub or sub.plan != 'pro':
                    if not credits:
                        credits = await Credits.get_user_credits(user.id)
                    if credits and credits.balance <= 0:
                        raise HTTPException(
                            status_code=402,
                            detail='شما به سقف مصرف رسیده‌اید. لطفاً برای شارژ به ربات مراجعه کنید.',
                        )

                if credits:
                    await Credits.deduct_credit(
                        user.id,
                        amount=1,
                        reference=f'chat:{uuid4()}',
                    )
        except HTTPException:
            raise
        except Exception as e:
            log.warning(f'ERA limit check failed, falling back to simple credits: {e}')
            from open_webui.models.subscriptions import Subscriptions
            from open_webui.models.user_credits import Credits
...
async def get_app_config(request: Request):
    ...
    if user:
        try:
            ...
            if await is_era_db_available():
                ...
                era = await _get_account_usage_internal(user)
                subscription_info = {
                    'plan': era.get('plan', 'free'),
                    'status': era.get('subscription', {}).get('status', 'active'),
                    'expires_at': era.get('subscription', {}).get('expires_at'),
                }
                limits = era.get('limits', {})
                five_hour = limits.get('five_hour', {})
                seven_day = limits.get('seven_day', {})
                credit_info = {
                    'balance': five_hour.get('remaining', 0),
                    'total_used': five_hour.get('used', 0),
                    'reset_date': five_hour.get('reset_at'),
                    'five_hour': five_hour,
                    'seven_day': seven_day,
                }
            else:
                ...
                cred = await Credits.get_user_credits(user.id)
                if cred:
                    credit_info = {
                        'balance': cred.balance,
                        'total_used': cred.total_used,
                        'reset_date': cred.reset_date,
                    }
        except Exception as e:
            log.warning(f'Error fetching ERA subscription info for config: {e}')
            from open_webui.models.subscriptions import Subscriptions
            from open_webui.models.user_credits import Credits
            sub = await Subscriptions.get_subscription_by_user_id(user.id)
            if sub:
    ...
    return {
        **({'onboarding': True} if onboarding else {}),
        ...
        'telegram_bot_username': TELEGRAM_BOT_USERNAME_PERSISTENT.value if TELEGRAM_BOT_TOKEN_PERSISTENT.value else None,
        'bale_bot_username': BALE_BOT_USERNAME_PERSISTENT.value if BALE_BOT_TOKEN_PERSISTENT.value else None,
        'features': {
            'auth': WEBUI_AUTH,
            ...
            'enable_bot_auth': bool(TELEGRAM_BOT_TOKEN_PERSISTENT.value or BALE_BOT_TOKEN_PERSISTENT.value),
...
async def get_current_usage(user=Depends(get_verified_user)):
    ...
    try:
        ...
        sub = await Subscriptions.get_subscription_by_user_id(user.id)
        result['plan'] = sub.plan if sub else 'free'
        result['subscription'] = {
        ...
        credits = await Credits.get_user_credits(user.id)
        result['quota'] = {
            'used': credits.total_used if credits else 0,
            'limit': 50,
            'remaining': credits.balance if credits else 50,
        }

        # Merge ERA (Dr. Boz) account data if available
        try:
            from open_webui.utils.era_db import is_era_db_available
            if await is_era_db_available():
                from open_webui.routers.account import get_account_usage
                era_data = await get_account_usage(user=user)
                if era_data.get('plan') and era_data['plan'] != 'free':
                    result['plan'] = era_data['plan']
                if era_data.get('limits'):
                    result['era_limits'] = era_data['limits']
                if era_data.get('wallet'):
                    result['era_wallet'] = era_data['wallet']
        except Exception as era_err:
            log.debug(f'ERA account data not available: {era_err}')
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            ...
            'description': f'{app.state.WEBUI_NAME} is an AI-powered chat platform by Dr. Boz.',
            'start_url': '/',
            'display': 'standalone',
