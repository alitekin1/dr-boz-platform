...
if JWT_EXPIRES_IN.value == '-1':
    log.warning(
        ...
        '    See: https://docs.drboz.com/reference/env-configuration\n'
    )
...
if CUSTOM_NAME:
    try:
        r = requests.get(f'https://api.drboz.com/api/v1/custom/{CUSTOM_NAME}')
        data = r.json()
        if r.ok:
            if 'logo' in data:
                WEBUI_FAVICON_URL = url = (
                    f'https://api.drboz.com{data["logo"]}' if data['logo'][0] == '/' else data['logo']
                )
            ...
            if 'splash' in data:
                url = f'https://api.drboz.com{data["splash"]}' if data['splash'][0] == '/' else data['splash']
...
# Bot Auth Settings
TELEGRAM_BOT_TOKEN_PERSISTENT = PersistentConfig(
    'TELEGRAM_BOT_TOKEN',
    'bot.telegram_token',
    os.environ.get('TELEGRAM_BOT_TOKEN', ''),
)
TELEGRAM_BOT_USERNAME_PERSISTENT = PersistentConfig(
    'TELEGRAM_BOT_USERNAME',
    'bot.telegram_username',
    os.environ.get('TELEGRAM_BOT_USERNAME', ''),
)
BALE_BOT_TOKEN_PERSISTENT = PersistentConfig(
    'BALE_BOT_TOKEN',
    'bot.bale_token',
    os.environ.get('BALE_BOT_TOKEN', ''),
)
BALE_BOT_USERNAME_PERSISTENT = PersistentConfig(
    'BALE_BOT_USERNAME',
    'bot.bale_username',
    os.environ.get('BALE_BOT_USERNAME', ''),
)
AUTH_CODE_TTL_PERSISTENT = PersistentConfig(
    'AUTH_CODE_TTL',
    'bot.auth_code_ttl',
    os.environ.get('AUTH_CODE_TTL', '300'),
)

# Billing Settings (Dr. Boz SAAS)
MODEL_PRICING_CONFIG = PersistentConfig(
    'MODEL_PRICING',
    'billing.model_pricing',
    os.environ.get('MODEL_PRICING', '{}'),
)

DEFAULT_COOLDOWN_LIMIT_TOMAN = PersistentConfig(
    'DEFAULT_COOLDOWN_LIMIT_TOMAN',
    'billing.default_cooldown_limit',
    os.environ.get('DEFAULT_COOLDOWN_LIMIT_TOMAN', '100000'),
)

DEFAULT_WEEKLY_LIMIT_TOMAN = PersistentConfig(
    'DEFAULT_WEEKLY_LIMIT_TOMAN',
    'billing.default_weekly_limit',
    os.environ.get('DEFAULT_WEEKLY_LIMIT_TOMAN', '1000000'),
)

DEFAULT_COOLDOWN_HOURS = PersistentConfig(
    'DEFAULT_COOLDOWN_HOURS',
    'billing.default_cooldown_hours',
    os.environ.get('DEFAULT_COOLDOWN_HOURS', '5'),
)
