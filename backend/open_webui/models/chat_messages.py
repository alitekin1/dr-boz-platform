...
def _extract_output_text(data: dict) -> str:
    """
    Pull plain text out of an assistant message for cost estimation when
    the provider didn't return token usage. Handles both the legacy
    string `content` field and the new structured `output` list used by
    Responses API / Open WebUI's middleware.
    """
    ...
    async def upsert_message(
        ...
        async with get_async_db_context(db) as db:
            ...
            if existing:
                ...
                spending_triggered = False
                if usage:
                    had_tokens = existing.usage and (existing.usage.get('input_tokens') or existing.usage.get('prompt_tokens'))
                    existing.usage = {**(existing.usage or {}), **usage}
                    if not had_tokens and data.get('done'):
                        spending_triggered = True
                elif data.get('done') and existing.role == 'assistant':
                    had_tokens = existing.usage and (existing.usage.get('input_tokens') or existing.usage.get('prompt_tokens'))
                    if not had_tokens:
                        spending_triggered = True
                ...
                if spending_triggered:
                    try:
                        from open_webui.utils.spending import record_spending_from_message
                        await record_spending_from_message(
                            user_id=user_id,
                        ...
                        log.warning(f'Failed to record spending for message {composite_id}: {e}')
                ...
                if (usage or data.get('done')) and message.role == 'assistant':
                    try:
                        from open_webui.utils.spending import record_spending_from_message
                        await record_spending_from_message(
                            user_id=user_id,
