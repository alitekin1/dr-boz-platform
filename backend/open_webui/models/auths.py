...
class MiniAppAuthForm(BaseModel):
    initData: str


class AuthCodeForm(BaseModel):
    code: str
    provider: str  # 'telegram' or 'bale'
    ...
    async def insert_new_auth(
        ...
        async with get_async_db_context(db) as db:
            ...
            user = await Users.insert_new_user(
                id, name, email, profile_image_url, role, oauth=oauth, phone=phone, db=db
            )
