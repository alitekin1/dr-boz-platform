...
class UserUpdateForm(BaseModel):
    ...
    def normalize_phone(cls, v: Optional[str]) -> Optional[str]:
        if v is None or v == '':
            return None
        s = str(v).strip()
        # Accept any of: +98XXXXXXXXXX, 00989..., 09XXXXXXXXX, 9XXXXXXXXX
        digits = re.sub(r'[^\d+]', '', s)
...
class UsersTable:
    async def insert_new_user(
        ...
        async with get_async_db_context(db) as db:
            ...
            result = User(**user.model_dump(exclude={'plan', 'credits', 'credits_used'}))
            db.add(result)
            await db.commit()
        ...
        async with get_async_db_context(db) as db:
            ...
            if filter:
                ...
                if order_by and order_by.startswith('group_id:'):
                    ...
                    if direction == 'asc':
                        stmt = stmt.order_by(User.phone.asc().nulls_last())
                    else:
                        stmt = stmt.order_by(User.phone.desc().nulls_last())
